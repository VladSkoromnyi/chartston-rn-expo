...
| **8 — Theming & polish** ✅ (code) | Production feel | dark/light **ThemeSelector** ✅; **chart types** line/area/bar/baseline + ChartTypeSelector ✅; **markers + price lines** (declarative props) ✅; **basic drawing tools** (trend + horizontal, data-coord, per-symbol; placement via crosshair in the harness) ✅ | states + theme | **code-verified** (lib tsc/eslint/29 tests + harness tsc + bundle 200); **device finger-test pending** (screenshots off per Vlad) → merge to develop after. In-chart drawing-mode toolbar = future/v2. |
