...
| **5 — WS engine + Binance** | Real live data | `stream/` per C.3, Binance REST+WS, backfill↔live merge, reconnect/heartbeat/resubscribe | reconnect group | BTCUSDT 1m live w/ backfill; survives forced disconnects. |
| **6 — Switch UI** | Generic UI | `SelectList→BottomSheet→onSelect` + PeriodSelector + SymbolSelector | switch group | switching instant, race-safe, leak-free. |
| **7 — Studies** | Bottom-line + overlays | Volume/RSI/MACD panes; SMA/EMA/Bollinger/VWAP overlays; `StudyDescriptor` built-ins; IndicatorMenu | indicator group | values match reference; panes recompute live; add/remove works. |
